...
# Load data
rndwlk = pd.read_csv('sequence_learning/sequence_learning_simulations/rndwlk_depth3_100trials.csv', header=None)
...
for block in range(1, Nblocks + 1):
    ...
    for trial in range(1, Ntrials_perblock + 1):
        ...
        state2 = choice1
        p_2 = np.exp(______ * Qval[:, ______ - 1, 1]) / np.sum(np.exp(______ * Qval[:, state2 - 1, 1]))
        choice2 = np.random.choice(np.arange(1, 3), p=______)
        
        state3 = -2 + 2 * choice1 + choice2
        p_3 = np.exp(______ * Qval[:, state3 - 1, 2]) / np.sum(np.exp(______ * Qval[:, state3 - 1, 2]))
        ...
        PE_2 = Qval[______, ______, ______] - Qval[______, ______, 1]
        PE_3 = ______ - Qval[choice3 - 1, state3 - 1, 2]
        
        Qval[choice1 - 1, state1 - 1, 0] += ______ * ______ + ______ * ______ * ______ + ______ * (______ ** 2) * ______
        Qval[choice2 - 1, state2 - 1, 1] += ______ * ______ + ______ * ______ * ______
        Qval[choice3 - 1, state3 - 1, 2] += ______ * ______

# Save data
df.to_pickle("sequence_learning/data/simulated_data.pkl")
...
# Save data for Stan
with open("sequence_learning/data/data_stan_format.pkl", "wb") as f:
