...
for (block in 1:Nblocks){
  ...
  for (trial in 1:Ntrials_perblock){
    ...
    state1  = 1
    p_1     = exp(______*Qval[, state1, 1]) / sum(exp(______*Qval[, state1, 1]))
    choice1 = sample(1:2,1,prob=p_1)
    
    state2  = choice1
    p_2     = exp(______*Qval[, ______, 2]) / sum(exp(______*Qval[, ______, 2]))
    choice2 = sample(1:2,1,prob=______)
    
    state3  = -2 + 2*choice1 + choice2
    p_3       = exp(______*Qval[, ______, 3]) / sum(exp(______*Qval[, ______, 3]))
    choice3 = sample(1:2,1,prob=______)
    ...
    PE_2 = Qval[______, ______, ______] - Qval[______, ______, ______]
    PE_3 = ______ - Qval[______, ______, ______]
    ...
    Qval[choice3, state3, 3] = Qval[choice3, state3, 3] + ______*______
    
    
  }
}     
save(df, file="sequence_learning/data/simulated_data.Rdata")
...
#save
save(data_for_stan, file="sequence_learning/data/data_stan_format.Rdata")
