...
modelfit_mcmc <-function(path, data_path, mymcmc){
  ...
  #load model
  load('two_step_task/stan/data/modelfit_compile.rdata')
  ...
  #save
  if (mymcmc$datatype=='empirical'){fit$save_object('two_step_task/stan/data/modelfit_empirical.rds')
    cat(paste0('[stan_modeling]:  "modelfit_empirical.rds" was saved at "',path$data,'"'))
  }
  if (mymcmc$datatype=='artificial'){fit$save_object('two_step_task/stan/data/modelfit_empirical.rds')
