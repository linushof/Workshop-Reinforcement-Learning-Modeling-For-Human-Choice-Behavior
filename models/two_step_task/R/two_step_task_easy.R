...
for (block in 1:Nblocks){
  ...
  Qmf1               = rep(,Narms) #stage 1 - only one state
  Qmf2               = matrix(0.5,,Narms) #stage 2 - two states
  ...
  for (trial in 1:Ntrials_perblock){
    ...
    Qmb1[1] = state_transition[1,1]*###+state_transition[2,1]*max(Qmf2[2,])
    Qmb1[2] = state_transition[1,2]*###+state_transition[2,2]*max(Qmf2[2,])
    
    #integrate MF and MB values modulated by omega parameter
    Qnet1     = ###*Qmb1+(1-omega)*Qmf1
    
    #make first-stage choice
    p1        = exp(###*Qnet1) / sum(exp(###*Qnet1))
    choice1   = sample(1:Narms,1,prob=p1)
    unchosen1 = c(1:Narms)[-choice1]
    
    #sample state transition based on first stage choice
    state=sample(1:###,1,prob=state_transition[,choice1])
    state_prob = state_transition[state,choice1]
    
    #second stage choice
    p2        = exp(beta*###) / sum(exp(beta*###))
    ...
    #Prediction erros
    PE2 = ### - Qmf2[state,choice2] #second stage reward-based prediction error
    PE1 = Qmf2[state,choice2]-Qmf1[choice1] #first stage prediction error
    
    #second stage update
    Qmf2[state,choice2]=Qmf2[state,choice2]+###*PE2
    #first stage update with an eligibility trace for the second-stage prediction error
    Qmf1[choice1] = Qmf1[choice1] + alpha*PE1+###*alpha*PE2
